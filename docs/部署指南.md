...
| `SEED_ON_START` | 设为 `true` 时，**仅在数据库尚无用户**时执行 seed；已有数据时自动跳过，重启不会清库 |
...
首次构建约 3–8 分钟（取决于网络）。**Docker 构建前**建议在 backend 执行 `pnpm seed:assets`（从前端同步素材到 `seed-assets/`，供容器内 seed 使用）。启动顺序：MySQL / Redis 健康检查通过 → 后端 `prisma db push` 同步表结构 → 若 `SEED_ON_START=true` 且库为空则 seed → 前端 Nginx。
...
| `SEED_ON_START` | false | 为 `true` 时仅在空库时 seed，不会每次重启清库 |
